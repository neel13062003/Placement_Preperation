...
public:
    ...
    int rob(vector<int>& nums) {
        ...
        // return max(solveRec(first,n-2),solveRec(second,n-2));
        
        // vector<int>dp(n+1,-1);
        // return max(solveMem(first,n-2,dp),solveMem(second,n-2,dp));
        
        // if(n<=0) return 0;
        // return max(solveTab(first,n-2),solveTab(second,n-2));
        
        if(n<=0) return 0;
        return max(solveSpace(first,n-2),solveSpace(second,n-2));
    }
};